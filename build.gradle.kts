...
val testJavaVersion =
  JavaLanguageVersion.of(
    if (testJavaVersionProperty != null) testJavaVersionProperty.toString()
    else JavaVersion.current().majorVersion
  )
...
//
// This is a method of an object rather than a function of the build script, because a task action
// that called a build script function would capture the build script, which the configuration
...
tasks.named<Javadoc>("javadoc") { strictJavadoc() }
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    // A local variable, because a task action that read the build script's `requireJavadocMarker`
    // would capture the build script, which the configuration cache cannot serialize.
